...
class Board:
    ...
    def __init__(self, pieces_remaining):
        self.board = [['.']*9 for _ in range(10)]
        #Generate all pieces
        Piece(self, pieces_remaining)

class Piece:
    def __init__(self, board, pieces_remaining):
...
class Engine(Board):
    ...
    def __init__(self):
        ...
        Board.__init__(self, self.pieces_remaining)
    ...
    #@staticmethod
    def find_correct(self, xpos, Piece, type, move):
        pieces = []
        print(len(self.pieces_remaining))
        ...
        '''if Piece == 'e' or Piece == 'E':
            if len(pieces) > 1:
                if pieces[0].posy == pieces[1].posy:
                    if move == '-':
                        if type == 'r':
                            if not piece.posx == 9:
        ...
        if len(pieces) > 1:
            if xpos == '+':
                if Piece != 's' and Piece != 'S':
                    ...
                    for i in range(len(pieces)):
                        for j in range(len(pieces)):
                            if i != j:
                                if pieces[i].posy == pieces[j].posy:
                                    new_pieces.append(pieces[i])
                                    new_pieces.append(pieces[j])
                                    print("soldier + more than 1")
                                    break
                ...
                if Piece != 's' and Piece != 'S':
                    ...
                    if type == 'r':
                        for piece in pieces:
                            if piece.posx > low.posx:
                                low = piece
                        piece_to_be_moved = low
                    elif type == 'b':
                        for piece in pieces:
                            if piece.posx < low.posx:
                                low = piece
                        ...
                        if move == '-':
                            ...
                            if type == 'r':
                                if piece.pname == 'E':
                                    ...
                                    if not piece.posx <= 7:
                                        piece_to_be_moved = piece
                                        print("redhelloadvise")
                                elif not piece.posx == 0:
                                    piece_to_be_moved = piece
                            if type == 'b':
                                if piece.pname == 'e':
                                    if not piece.posx >= 4:
        ...
        def input_helper():
            if Piece >= 'a' and Piece <= 'z':
                ...
                if FormerFile == '+' or FormerFile == '-':
                    piece = self.find_correct(FormerFile, Piece, 'b', Operator)
                else:
                    piece = self.find_correct(FormerFile - 1, Piece, 'b', Operator)
            elif Piece >= 'A' and Piece <= 'Z':
                color = 'red'
                if FormerFile == '+' or FormerFile == '-':
                    piece = self.find_correct(FormerFile, Piece, 'r', Operator)
                else:
            ...
            if captures:
                for p in self.pieces_remaining:
                    if p.pname != piece.pname:
                        if p.posx == piece.posx and p.posy == piece.posy:
                            to_remove = p
                            break

                self.pieces_remaining.remove(to_remove)
        input_helper()
        '''
        if(Piece=='A'):
            #Advisor
