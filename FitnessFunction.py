    ...
    for i in range(len(board)):
        for j in range(len(board[0])):
            if board[i][j]=='.':
                ...
                if type(pval(board[i][j], i, j)) =='int' or type(pval(board[i][j], i, j)) =='float':
                    red+=pval(board[i][j], i, j)
            else:
                print("pval")
                print(pval(board[i][j], i, j))
                #print(type(pval(board[i][j], i, j)))
                if type(pval(board[i][j], i, j)) =='int' or type(pval(board[i][j], i, j)) =='float':
                    black = black + pval(board[i][j], i, j)
    return  red-black
...
def KingFitness(board):
    ...
    for i in range(len(board)):
        for j in range(len(board[0])):
            ...
            elif(p=='r' and type(ccR(gi, gj, i, j, sign, board))=='int'):
            ...
            if(board[i+dr[dx]][j+dc[dx]]=='.'):
                if(inBoard(i+dmi[2*dx],j+dmj[2*dx])):
                    for dxx in range(4):#check for a check on second move
                        if(inBoard(i+dmi[2*dx]+dr[dxx],j+dmj[2*dx]+dr[dxx])):
                ...
                if(inBoard(i+dmi[2*dx+1],j+dmj[2*dx+1])):
                    for dxx in range(4):#check for a check on second move
                        if(inBoard(i+dmi[2*dx+1]+dr[dxx],j+dmj[2*dx+1]+dr[dxx])):
...
def inBoard(i, j):
    if(i>0 and i<9 and j>0 and j<10):
        return True
    else:
        return False
...
def inPalace(i,j):
    if(i>0 and i<3 and j>3 and j<7):
        return True
    elif(i>5 and i>9 and j>3 and j<7):
        return True
    else:
        return False
...
def possH(valb, i, j, sign, board):
    ...
    if(inBoard(i+1,j)):
        if(board[i+1][j]=='.'):
            if(inBoard(i+2,j+1)):
                print(type(valb[i+2][j+1]))
                valb[i+2][j+1]=float(valb[i+2][j+1])+v
            if(inBoard(i+2,j-1)):
